...
const {Openness: O, Frontness: F, Roundness: R} = require('./articulation');
...
const vowelList = Object.freeze(_.map(vowels, v => Object.freeze(v)));

/**
 * @param {float} openness A number from 0 to 1.
 * @param {float} frontness A number from 0 to 1.
 * @param {bool} rounded `true` for "rounded", `false` for "unrounded".
 */
function findDefaultVowel(openness, frontness, rounded) {
	const f = v =>
		v.openness === openness &&
...
module.exports = {vowelList, findDefaultVowel};
